<script lang="ts">
	...
	import { ThumbsUp, ThumbsDown, Heart, Pin, CornerDownRight, MoreVertical, AlertTriangle } from 'lucide-svelte';
	...
	const bannedKeywords = [
		...
		'обнаженный', 'nude', 'naked', 'голый', 'голая'
	];
	...
	function filterComments(commentsList: any[]): any[] {
		return commentsList.map(comment => {
			const isBanned = containsBannedContent(comment.content);
			
			// Фильтруем replies
			const filteredReplies = comment.replies ? 
				comment.replies.map((reply: any) => ({
					...reply,
					isBanned: containsBannedContent(reply.content),
					filteredContent: containsBannedContent(reply.content) 
						? (lang === 'ru' 
							? 'Комментарий удален за нарушение правил платформы' 
							: 'Comment removed for platform policy violation')
						: reply.content
				})) 
				: [];
	...
	async function loadComments() {
		...
		if (res.ok) {
			...
			comments = filterComments(data.comments);
	...
	async function handleSubmit(e: Event) {
		...
		if (containsBannedContent(newComment)) {
			alert(lang === 'ru' 
				? 'Комментарий содержит запрещенный контент и не может быть опубликован'
				: 'Comment contains prohibited content and cannot be published');
			return;
		}
	...
	async function handleReply(parentId: number) {
		...
		if (containsBannedContent(replyContent)) {
			alert(lang === 'ru' 
				? 'Ответ содержит запрещенный контент и не может быть опубликован'
				: 'Reply contains prohibited content and cannot be published');
			return;
		}
	...
	async function handleLike(commentId: number, type: 'like' | 'dislike') {
		...
		// Не позволяем лайкать забаненные комментарии
		const comment = findCommentById(commentId);
		if (comment?.isBanned) return;
	...
	async function handleEditComment(commentId: number) {
		...
		// Проверка на запрещенный контент при редактировании
		if (containsBannedContent(editContent)) {
			alert(lang === 'ru' 
				? 'Комментарий содержит запрещенный контент и не может быть сохранен'
				: 'Comment contains prohibited content and cannot be saved');
			return;
		}
	...
	function findCommentById(commentId: number): any {
		for (const comment of comments) {
...
<div class="comments">
	...
	<div class="comments-list">
		{#each comments as comment (comment.id)}
			<div class="comment" class:pinned={comment.is_pinned} class:banned={comment.isBanned}>
				...
				<img src={comment.avatar} alt={comment.username} class="avatar" class:banned-avatar={comment.isBanned} />
				<div class="comment-content">
					...
					{:else}
						<p class:deleted-text={comment.isBanned}>{comment.filteredContent}</p>
					{/if}
					...
					{#if replyingTo === comment.id && !comment.isBanned}
					...
					{#if comment.replies && comment.replies.length > 0}
						<div class="replies">
							{#each comment.replies as reply (reply.id)}
								<div class="comment reply" class:banned={reply.isBanned}>
									<img src={reply.avatar} alt={reply.username} class="avatar small" class:banned-avatar={reply.isBanned} />
									<div class="comment-content">
										<div class="comment-header">
										...
										<p class:deleted-text={reply.isBanned}>{reply.filteredContent}</p>
